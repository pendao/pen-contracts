...
contract PenDaoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPEN }
    ...
    address public sPEN;
    uint public sPENQueue; // Delays change to sPEN address
    ...
    constructor (
       ...
       isLiquidityToken[ _PENUSDT ] = true;
       liquidityTokens.push( _PENUSDT );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for PEN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PEN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PEN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PEN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPENERC20( PEN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPEN ).balanceOf( msg.sender ); // Can only borrow against sPEN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PEN
        @param _amount uint
     */
    function repayDebtWithPEN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPENERC20( PEN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PEN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PEN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PEN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PEN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PEN decimals
            value_ = _amount.mul( 10 ** IERC20( PEN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPEN ) { // 9
            sPENQueue = block.number.add( blocksNeededForQueue );
        } else return false;
