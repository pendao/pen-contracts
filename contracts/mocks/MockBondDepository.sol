...
contract MockPenDaoBondDepository is Ownable {
  ...
  address public immutable PEN; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints PEN when receives principle
  ...
  struct Bond {
    uint256 payout; // PEN remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 PEN ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) PEN
         */
    ...
    if (fee != 0) {
      ...
      IERC20(PEN).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(PEN).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(PEN).approve(stakingHelper, _amount);
        IStakingHelper(stakingHelper).stake(_amount, _recipient);
      } else {
        IERC20(PEN).approve(staking, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(PEN).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to PEN supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(PEN).totalSupply();
  ...
  /**
   *  @notice calculate amount of PEN available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or PEN) to the DAO
   *  @return bool
   */
