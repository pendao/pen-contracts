...
contract MockPenDaoTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sPEN;
  uint256 public sPENQueue; // Delays change to sPEN address
  ...
  constructor(
    ...
    // isLiquidityToken[ _PENDAI ] = true;
    // liquidityTokens.push( _PENDAI );
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sPEN).balanceOf(msg.sender); // Can only borrow against sPEN held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithPEN(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(PEN).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SPEN) {
      // 9
